...
#### 5.2.1 卡片空间分配（2026-07-24 批准）

卡片总高度和同屏密度基本保持不变，信息区不得挤占迷你 K 线的主要空间：

1. 股票代码、名称、价格、涨跌幅、行情时间、成功时间和控制按钮压缩为
   两行紧凑信息区；
2. 行情时间和成功时间使用精简日期时间格式，但不得删除；
3. 五周期按钮继续全部可见，并使用紧凑高度；
4. 迷你 K 线高度由 96 像素提高到 180 像素，成为卡片中占比最大的区域；
5. 最新中文操作和形态信息继续固定在卡片底部；
6. 不改变趋势线、买卖点、五周期状态或数据新鲜度的计算与语义。
