...
describe('DowStrategyCard', () => {
  ...
  it('runs selection, shows five period match positions and backtests the selected stock', async () => {
    const fetchMock = vi.fn(async (url: string) => {
      ...
      return { ok: true, json: async () => ({ stocks: [{ symbol: '700.HK', name: '腾讯控股', strategyScore: 82, triggerTimeframes: ['30m', 'day'], localTriggerTimeframes: ['30m'], longTermTriggerTimeframes: ['day'], formalTimeframes: ['30m'], provisionalTimeframes: ['day'], dataFreshness: 'partial' }] }) }
    })
    render(<DowStrategyCard market="hk" fetcher={fetchMock as any} />)
    await userEvent.click(screen.getByRole('button', { name: '执行选股' }))
    expect(await screen.findByText(/700.HK/)).toBeInTheDocument()
    expect(screen.getByText('局部 30m · 长期 日线')).toBeInTheDocument()
